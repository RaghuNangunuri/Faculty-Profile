...
$conn=mysqli_connect($server,$uname,$pass,$dbname);
  $sql="select * from personal where emailid='$uid'";
  $result=mysqli_query($conn,$sql);
  if($personalrow=mysqli_fetch_row($result))
...
<body>
  ...
  <?
  $select_path="select * from imagedisplay where emailid='$uid'";

if($var=mysqli_query($conn,$select_path))
$n=mysqli_num_rows($var);
...
{
 $image_path=$row[1];
 $image_name=basename($row["imagepath"]);
  ...
  <header id="header">
    <div class="d-flex flex-column">
      ...
      <div class="profile">
        ...
        <?
        $sql="select * from social where emailid='$uid'";
        if($result=mysqli_query($conn,$sql))
         $socialrow=mysqli_fetch_row($result);
        ?>
